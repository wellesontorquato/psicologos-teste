...
<section style="padding: 40px 20px; background: #fff;">
    <div style="max-width: 800px; margin: auto;">
        ...
        {{-- Imagem de capa (PADRÃO MOBILE + DESKTOP) --}}
        @if ($news->image)
...
@push('styles')
<style>
    /* ===== Capa (corrige tamanho + alinhamento) ===== */
    .cover-figure {
        margin: 0 0 26px 0;
    }

    /* container garante alinhamento com o texto (mesma largura) */
    .cover-media {
        width: 100%;
        border-radius: 12px;
        overflow: hidden;
        background: #f3f3f3;
        /* no mobile mantém proporção editorial e evita ficar “gigante” */
        aspect-ratio: 16 / 9;
    }
    ...
    /* Desktop: limita altura para não “empurrar” o conteúdo */
    @media (min-width: 992px) {
        .cover-media {
            max-height: 360px; /* ajuste fino: 320–420 costuma ficar ótimo */
            aspect-ratio: auto; /* deixa a altura ser governada pelo max-height */
        }

        .cover-image {
            height: 360px; /* combina com o max-height acima */
        }
    }
    ...
    /* imagens dentro do conteúdo (mantém seu padrão) */
    .noticia-conteudo img {
        max-width: 280px;
        border-radius: 10px;
        object-fit: cover;
